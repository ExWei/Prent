...
function Prent() {
    ...
    function createConsoleString(args, callback) {
        var message;
        if (args.message['1'] !== undefined) {
            message = Array.prototype.slice.call(args.message);
        } else {
        ...
        if (message instanceof Array === false) {
            ...
            message = util.format(message[0], message.slice(1));
        }
        var data = util.format("[%s%s%s] [%s]: %s".replace(/\[/g, "\x1B[1m[\x1B[22m").replace(/\]/g, "\x1B[1m]\x1B[22m"), colors[args.level][0], args.level, colors[args.level][1], args.datetime, message);
        callback(data);
    }
    ...
    this.trace = function (message) {
        write({level: "Trace", message: arguments, datetime: new Date().toUTCString()});
    };

    this.debug = function (message) {
        write({level: "Debug", message: arguments, datetime: new Date().toUTCString()});
    };

    this.info = function (message) {
        write({level: "Info", message: arguments, datetime: new Date().toUTCString()});
    };

    this.warn = function (message) {
        write({level: "Warn", message: arguments, datetime: new Date().toUTCString()});
    };

    this.error = function (message) {
        write({level: "Error", message: arguments, datetime: new Date().toUTCString()});
    };
}
